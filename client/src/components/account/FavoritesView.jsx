...
const FavoritesView = ({ user, onUpdateUser, setSearchValue, setActiveView, setShowMapOverlay, setIsSearching, setMapOverlayMode }) => {
  // Use user.clientProfile.favorites as the source of truth
  const favorites = user?.clientProfile?.favorites || [];

  const removeFavorite = async (e, address) => {
    e.stopPropagation();
    ...
    try {
      ...
      if (data.success) {
        if (onUpdateUser) onUpdateUser(data.data);
      }
    } catch (err) {
